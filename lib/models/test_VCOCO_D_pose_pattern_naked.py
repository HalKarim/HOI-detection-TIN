...
def im_detect_GUI_H(sess, net, im_orig,im_shape, image_id,image_xy,Test_RCNN, prior_mask, Action_dic_inv, object_thres, human_thres, prior_flag,
              detection):
    #im_orig, im_shape = get_blob(image_id)

    blobs = {}
    blobs['H_num'] = 1
    #print(Test_RCNN[image_id])
    for Human_out in Test_RCNN[image_id]:
        #print(np.max(Human_out[5]))
        if (np.max(Human_out[5]) > human_thres) and (Human_out[1] == 'Human') \
                and Human_out[2][0]>image_xy[0] - 50 and Human_out[2][1]>image_xy[1] - 50 \
                and Human_out[2][2]<image_xy[2] + 50 and Human_out[2][3]<image_xy[3] + 50 :
...
def im_detect_GUI_O(sess, net, im_orig,im_shape, image_id,image_xy,Test_RCNN, prior_mask, Action_dic_inv, object_thres, human_thres, prior_flag,
              detection):
    #im_orig, im_shape = get_blob(image_id)
        ...
        if (np.max(Human_out[5]) > human_thres) and (Human_out[1] == 'Human') :
            ...
            for Object in Test_RCNN[image_id]:
                ...
                if (np.max(Object[5]) > object_thres) and not (np.all(Object[2] == Human_out[2])) \
                        and Object[2][0] > image_xy[0] - 15 and Object[2][1] > image_xy[1] - 15 \
                        and Object[2][2] < image_xy[2] + 15 and Object[2][3] < image_xy[3] + 15 : # This is a valid object
...
def im_detect_GUI(sess, net, im_orig,im_shape, image_id,Test_RCNN, prior_mask, Action_dic_inv, object_thres, human_thres, prior_flag,
              detection):
    #im_orig, im_shape = get_blob(image_id)
