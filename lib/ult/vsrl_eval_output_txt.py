...
def _load_vcoco(vcoco_file):
  ...
  #image_ids = np.loadtxt(open('E:/projects/Transferable-Interactiveness-Network/Data/v-coco/data/splits/vcoco_val_out.ids', 'r'),dtype=int)
  image_ids = np.loadtxt(open('E:/projects/Transferable-Interactiveness-Network/Data/v-coco/data/splits/vcoco_test.ids', 'r'), dtype=int)
  #np.set_printoptions(threshold=sys.maxsize)E:\projects\Transferable-Interactiveness-Network\Data\v-coco\data\splits
  #print(np.sort(image_ids))
  ...
  for i in range(len(vsrl_data)):
    ...
    role_j=(vsrl_data[i]['role_name'])
    act_j=(vsrl_data[i]['action_name'])
    role_obid_j=(vsrl_data[i]['role_object_id'])
    include_j=(vsrl_data[i]['include'])
    #dic['role_object_id'] =role_obid_j.append(vsrl_data[i]['role_object_id'])
    #print(dic['role_object_id'])
    vsrl_data_out_j = []
    #for a in image_ids:
    #print(len(np.unique(vsrl_data[i]['image_id'])))
    for b in range(len(vsrl_data[i]['image_id'])):
      if vsrl_data[i]['image_id'][b] in image_ids:
        vsrl_data[i]['image_id'] = np.array(vsrl_data[i]['image_id']).reshape((-1, 1))
        #print(vsrl_data[i]['image_id'][b])
        vsrl_data_out_j.append(vsrl_data[i]['image_id'][b].tolist())
        #print(vsrl_data_out_j)
    #vsrl_data_out_jj.append(np.array(vsrl_data_out_j))
    vsrl_data_out_jjj=np.array(vsrl_data_out_j)
    label_j = []
    vsrl_data[i]['ann_id'] = np.array(vsrl_data[i]['ann_id']).reshape((-1, 1))
    vsrl_data[i]['label'] = np.array(vsrl_data[i]['label']).reshape((-1, 1))
    ann_id_j=(vsrl_data[i]['ann_id'])
    for f in range(len(vsrl_data[i]['label'])):
        label_j.append(vsrl_data[i]['label'][f])
          #label_j=label_j.tolist()
          #print(label_j)
    label_jj=(np.array(label_j))
    #for f in range(len(vsrl_data[i]['label'])):
        #label_jj.append(label_j[f:f+1])
        #role_j.append(vsrl_data[i]['role_name']),'role_object_id','role_name','action_name'
        #act_j.append(vsrl_data[i]['action_name'])

    #dic['role_name'] = role_j
    #dic['action_name'] = act_j
    #print(dic['role_object_id'])
    dic['image_id'] = vsrl_data_out_jjj
    dic['ann_id'] = ann_id_j
    dic['role_object_id'] = role_obid_j
    dic['label'] = label_jj
    dic['action_name'] = act_j
    dic['role_name'] = role_j
    dic['include'] = include_j


    detection_j.append(dic)
