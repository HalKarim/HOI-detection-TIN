...
def generate_pkl_GUI( test_D, test_result, prior_mask, Action_dic_inv, sigmoid_coeff, prior_flag=3):
    generate_result = []
    for tr in range(len(test_result)):
        #print(tr)
        dic = test_result[tr]
        ...
        dic_new['object_box'] = bbox_O
        dic_new['object_class'] = class_O
        dic_new['O_det'] = dic['O_det']
        dic_new['H_det'] = dic['H_det']
        ...
        for i in range(29):
            ...
            # perform nis on target object
            elif (score_binary_d[max_idx[i]][0] < inter_bound and score_binary_d[max_idx[i]][1] > no_inter_bound) or \
                    class_O[max_idx[i]] == 1:
                dic_new[Action_dic_inv[i]] = np.append(np.full(4, np.nan).reshape(1, 4), 0.0)
...
if __name__ == '__main__':
    ...
    vcocoeval      = VCOCOeval(cfg.DATA_DIR + '/' + 'v-coco/data/vcoco/vcoco_test.json', cfg.DATA_DIR + '/' + 'v-coco/data/instances_vcoco_all_2014.json', cfg.DATA_DIR + '/' + 'v-coco/data/splits/vcoco_val_out.ids')
    mode = args.Mode
    # now_best_vcoco_S
    input_S = args.S_pkl
    output_dir = cfg.ROOT_DIR+'/-Results/'
    
    output_file =  'CVPR_best_VCOCO_Mytest15000_out_0524'
                   #_out_Mytest15000'
